...
public class SendPhoneCode
{
	...
	public static void sendCode(String phone,String code)
	{
		...
		// 单条发送
		singleSend(userid, pwd,isEncryptPwd,phone,code);
		// 每次请求想要获取的上行的最大条数。
		int retsizeMo = 100;
		//实例化获取上行线程
	 ...
	 */
	public static void singleSend(String userid, String pwd,boolean isEncryptPwd,String phone ,String code)
	{
		try
		{
			// 参数类
			...
			// 设置内容
			message.setContent("您的验证码是"+code+"，在1分钟内输入有效。如非本人操作请忽略此短信。");
			// 设置扩展号
			message.setExno("11");
			// 用户自定义流水编号
